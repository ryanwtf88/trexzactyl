...
namespace Trexzactyl\Http\Controllers\Api\Remote\Servers;
...
use Trexzactyl\Events\Server\Installed as ServerInstalled;
use Illuminate\Contracts\Events\Dispatcher as EventDispatcher;
use Trexzactyl\Http\Requests\Api\Remote\InstallationDataRequest;
...
class ServerInstallController extends Controller
{
    ...
    public function store(InstallationDataRequest $request, string $uuid): JsonResponse
    {
        ...
        if ($isInitialInstall && config()->get('Trexzactyl.email.send_install_notification', true)) {
            $this->eventDispatcher->dispatch(new ServerInstalled($server));
        } elseif (!$isInitialInstall && config()->get('Trexzactyl.email.send_reinstall_notification', true)) {
            $this->eventDispatcher->dispatch(new ServerInstalled($server));
        }
