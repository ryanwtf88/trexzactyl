...
namespace Trexzactyl\Services\Allocations;

use Trexzactyl\Models\Server;
use Webmozart\Assert\Assert;
use Trexzactyl\Models\Allocation;
use Trexzactyl\Exceptions\Service\Allocation\AutoAllocationNotEnabledException;
use Trexzactyl\Exceptions\Service\Allocation\NoAutoAllocationSpaceAvailableException;
...
class FindAssignableAllocationService
{
    ...
    {
        if (!config('Trexzactyl.client_features.allocations.enabled')) {
        ...
        // server.
        /** @var \Trexzactyl\Models\Allocation|null $allocation */
    ...
    {
        $start = config('Trexzactyl.client_features.allocations.range_start', null);
        $end = config('Trexzactyl.client_features.allocations.range_end', null);
        ...
        /** @var \Trexzactyl\Models\Allocation $allocation */
        $allocation = $server->node->allocations()
            ->where('ip', $server->allocation->ip)
            ->where('port', $port)
            ->firstOrFail();
