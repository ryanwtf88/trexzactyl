...
@section('footer-scripts')
    ...
    $(document).ready(function () {
        $('#pEggId').select2({placeholder: 'Select a Nest Egg'}).on('change', function () {
            ...
            var parentChain = _.get(Trexzactyl.nests, $("#pNestId").val());
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (objectChain.id === parseInt(Trexzactyl.server.egg_id) && Trexzactyl.server.image == opt.value) {
                    opt.selected = true
                }
            ...
            if (objectChain.id === parseInt(Trexzactyl.server.egg_id)) {
                if ($('#pDockerImage').val() != Trexzactyl.server.image) {
                    $('#pDockerImageCustom').val(Trexzactyl.server.image);
                }
            }
            ...
            $.each(_.get(objectChain, 'variables', []), function (i, item) {
                var setValue = _.get(Trexzactyl.server_variables, item.env_variable, item.default_value);
        ...
        $('#pNestId').select2({placeholder: 'Select a Nest'}).on('change', function () {
            $('#pEggId').html('').select2({
                data: $.map(_.get(Trexzactyl.nests, $(this).val() + '.eggs', []), function (item) {
                    return {
                        id: item.id,
                        text: item.name,
                    };
            ...
            if (_.isObject(_.get(Trexzactyl.nests, $(this).val() + '.eggs.' + Trexzactyl.server.egg_id))) {
                $('#pEggId').val(Trexzactyl.server.egg_id);
            }
